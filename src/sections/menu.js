...
export async function renderMenuManagement(main, editingItemId = null) {
  ...
  let html = `
    ...
    <div id="itemFormModal" class="modal">
      <div class="modal-content">
        ...
        <form id="itemAddForm" class="order-config-form">
          ...
          <textarea id="itemAddVariantKeywordsHindi" placeholder="Variant Keywords (Hindi, comma-separated keywords, blocks for each variant separated by '|', e.g. छोटा,नन्हा | बड़ा,विशाल)"></textarea>
   ...
   `).join("")}
    ...
    if (editingItem) {
      ...
      // START UPDATED LOGIC for Variants and Modifiers to show placeholders
      const variantsVal = editingItem.variants?.map(v => `${v.name}/${v.price}`).join(', ');
      document.getElementById('itemAddVariants').value = variantsVal || '';
      const variantsHindiVal = editingItem.variants?.map(v => `${v.nameHindi}/${v.price}`).join(', ');
      document.getElementById('itemAddVariantsHindi').value = variantsHindiVal || '';
      
      const modifiersVal = editingItem.modifiers?.map(m => `${m.name}/${m.price}`).join(', ');
      document.getElementById('itemAddModifiers').value = modifiersVal || '';
      const modifiersHindiVal = editingItem.modifiers?.map(m => `${m.nameHindi}/${m.price}`).join(', ');
      document.getElementById('itemAddModifiersHindi').value = modifiersHindiVal || '';

      // START UPDATED LOGIC for keywords
      const variantKeywordsVal = editingItem.variants?.map(v => v.variantKeywords?.join(', ') || '').join(' | ');
      document.getElementById('itemAddVariantKeywords').value = variantKeywordsVal || '';
      const variantKeywordsHindiVal = editingItem.variants?.map(v => v.variantKeywordsHindi || '').join(' | ');
      document.getElementById('itemAddVariantKeywordsHindi').value = variantKeywordsHindiVal || '';
      
      const modifierKeywordsVal = editingItem.modifiers?.map(m => m.modifierKeywords?.join(', ') || '').join(' | ');
      document.getElementById('itemAddModifierKeywords').value = modifierKeywordsVal || '';
      const modifierKeywordsHindiVal = editingItem.modifiers?.map(m => m.modifierKeywordsHindi || '').join(' | ');
      document.getElementById('itemAddModifierKeywordsHindi').value = modifierKeywordsHindiVal || '';
      // END UPDATED LOGIC
    ...
    if (window.pramukhIME) {
      ...
      const fields = [
        "itemAddNameHindi",
        "itemAddVariantsHindi",
        "itemAddVariantKeywordsHindi",
        "itemAddModifiersHindi",
        "itemAddModifierKeywordsHindi"
      ];
    
      document.getElementById('toggleHindiTyping').addEventListener('change', (e) => {
        const enableHindi = e.target.checked;
        console.log(`[IME] Toggle: ${enableHindi ? 'ON' : 'OFF'}`);
        ...
        if (enableHindi) {
          ...
          fields.forEach(id => {
            ...
            console.log(`[IME] Enabling Hindi on: ${id}`);
            ime.enable(el);
          });
        } else {
          fields.forEach(id => {
            const el = document.getElementById(id);
            if (!el) return;
            console.log(`[IME] Disabling Hindi on: ${id}`);
            ime.disable(el); // This actually disables IME
          });
        }
      });
    }
  ...
  const translateFieldPairs = [
    {eng: "itemAddName", hindi: "itemAddNameHindi"},
  ...
  document.getElementById("itemAddForm").onsubmit = async (e) => {
   ...
   // START MODIFIED parsePairs function
    const parsePairs = (str, hindiStr, keywordsStr = '', keywordsHindiStr = '', isModifier = false) => {
      ...
      return enParts.map((e, i) => {
        ...
        const itemKeywords = engKeywordBlocks[i] ? engKeywordBlocks[i].split(',').map(k => k.trim()).filter(k => k.length > 0) : [];
        const itemHindiKeywords = hindiKeywordBlocks[i] ? hindiKeywordBlocks[i] : '';
        
        // Use separate keys for variants and modifiers
        const keywordKey = isModifier ? 'modifierKeywords' : 'variantKeywords';
        const keywordHindiKey = isModifier ? 'modifierKeywordsHindi' : 'variantKeywordsHindi';

        return {
          name: name.trim(),
          nameHindi: nameHindi.trim(),
          price: Number(price),
          [keywordKey]: itemKeywords,
          [keywordHindiKey]: itemHindiKeywords
        };
    ...
    // Pass a flag to parsePairs to differentiate between variants and modifiers
    const variants = variantsStr ? parsePairs(variantsStr, variantsHindiStr, variantKeywordsStr, variantKeywordsHindiStr) : [];
    const modifiers = modifiersStr ? parsePairs(modifiersStr, modifiersHindiStr, modifierKeywordsStr, modifierKeywordsHindiStr, true) : [];
    ...
    for (const variant of variants) {
      ...
      await dataService.put("variants", variant);
    ...
    for (const modifier of modifiers) {
      ...
      await dataService.put("modifiers", modifier);
    ...
    await dataService.put("menuItems", itemData);
  ...
  document.querySelectorAll(".deleteItemBtn").forEach(btn => {
    btn.onclick = async () => {
      // MANDATORY FIX: `confirm()` will not work in this environment.
      // You must implement a custom modal UI for confirmation.
      // For now, the delete action is commented out.
      // if (confirm("Delete this item?")) {
      //   await POSDatabase.delete("menuItems", btn.dataset.id);
      //   renderMenuManagement(main);
      // }
      await dataService.delete("menuItems", btn.dataset.id);
      renderMenuManagement(main);
    };
  });
}
