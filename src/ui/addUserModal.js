...
// Helper function to generate the random 4-character alphanumeric suffix
  ...
  form.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (allUsers.some(u => u.username.toLowerCase() === username.toLowerCase())) {
       ...
       // --- NEW: Generate and check uniqueness for EPOS-FZJT ID locally ---
       let cashRegisterId;
       let isUniqueLocally = false;
       let attempts = 0;
       const MAX_ATTEMPTS = 10; // Prevent infinite loops in rare collision cases
       ...
       while (!isUniqueLocally && attempts < MAX_ATTEMPTS) {
           ...
           // Check if this generated ID already exists among local users
       ...
       // --- END NEW: EPOS-FZJT ID generation ---
      ...
      alert(`User "${username}" added successfully with Cash Register ID: ${cashRegisterId}!`);
      closeOrderModal();
