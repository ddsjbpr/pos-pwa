...
// Enable or disable all modifier checkboxes
...
if (hasVariants) {
  // If a variant is pre-selected (like the first one), enable everything
  ...
  if (selectedVariant) {
    ...
    if (modifiersSection) modifiersSection.classList.remove('hidden'); // Show always
    setModifiersEnabled(false);
    if (addToCartButton) addToCartButton.disabled = true;
  }
} else {
  // No variants → enable everything by default
  if (modifiersSection) modifiersSection.classList.remove('hidden');
  setModifiersEnabled(true);
  if (addToCartButton) addToCartButton.disabled = false;
}

// Event listener for variant selection (radio change)
